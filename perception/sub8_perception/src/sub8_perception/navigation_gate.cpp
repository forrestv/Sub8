...
Sub8NavigationGateDetector::Sub8NavigationGateDetector() : nh("~"), timeout_for_found_(2), image_transport_(nh), tf_listener_(tf_buffer_)
{
  ...
  blur_size_ = nh.param<int>("blur_size_", 3);
  ...
  vision_request_service_ =
      nh.advertiseService("/vision/navigation_gate/pose", &Sub8NavigationGateDetector::vision_request_cb, this);
  active_service_ =
      nh.advertiseService("/vision/navigation_gate/enable", &Sub8NavigationGateDetector::set_active_enable_cb, this);

  run();
}

std::vector<cv::Point> Sub8NavigationGateDetector::get_2d_feature_points(cv::Mat image)
{
  // Filter the image
...
void Sub8NavigationGateDetector::run()
{
  ros::Rate loop_rate(refresh_rate_);
  while (ros::ok())
  {
    if (active_)
      determine_navigation_gate_position();
    loop_rate.sleep();
    ros::spinOnce();
  }
  return;
...
void Sub8NavigationGateDetector::determine_navigation_gate_position()
{
...
bool Sub8NavigationGateDetector::set_active_enable_cb(std_srvs::SetBool::Request &req, std_srvs::SetBool::Response &res)
{
  active_ = req.data;
  res.success = true;
  return true;
...
bool Sub8NavigationGateDetector::vision_request_cb(sub8_msgs::VisionRequest::Request &req,
...
cv::Mat Sub8NavigationGateDetector::process_image(cv::Mat &image)
{
  ...
  cv::blur(lab_channels[0], processed_image, cv::Size(blur_size_, blur_size_));
...
double Sub8NavigationGateDetector::get_angle(cv::Point a, cv::Point b, cv::Point c)
{
  auto ba = a - b;
  auto bc = c - b;
...
bool Sub8NavigationGateDetector::valid_contour(std::vector<cv::Point> &contour)
{
...
std::vector<cv::Point> Sub8NavigationGateDetector::contour_to_2d_features(std::vector<std::vector<cv::Point>> &contour)
{
...
void Sub8NavigationGateDetector::visualize_k_gate_normal()
{
...
void Sub8NavigationGateDetector::visualize_3d_points_rviz(const std::vector<Eigen::Vector3d> &feature_pts_3d)
{
...
std::vector<cv::Point> Sub8NavigationGateDetector::get_corner_center_points(const std::vector<cv::Point> &features)
{
