...
class StartGateFinder():
	...
	def _img_callback_a(self, img):
		...
		image_clahe_hsv = cv2.cvtColor(limg, cv2.COLOR_BGR2HSV)

		image_b = cv2.medianBlur(image_clahe,5)
		cv2.imshow("original", image_clahe_hsv[:,:,1])

		image_clahe_hsv = cv2.medianBlur(image_clahe_hsv, 3)
		canny_hsv_bleh = cv2.Canny(image_clahe_hsv[:,:,1], 60, 60*5)

		cv2.imshow("asasdoiskdjaod", canny_hsv_bleh)
